...
using DungeonGenerator.Scripts.DUtils;
using UnityEngine;
        ...
        private void GenerateNextRoom() {
            ...
            do
            {
                ...
                if(doors == 1 && possibleRooms.Count > 1) {
                    ...
                    if (randomValue < chance|| roomsCount < 10) {//i added min amount by 10
                        ...
                        if (doors == 1 && possibleRooms.Count > 1) {
                            ...
                            if (randomValue2 < chance2) { //i added min amount by 10
                                r = random.range(0, possibleRooms.Count - 1);
                                roomToTry = possibleRooms[r].gameObject;
                            } else {
                ...
                if (!overlap) {
                    ...
                    for (int i = 0; i < ro.doors.Count; i++) {
                        ...
                        if (globalVoxels.ContainsKey(RoundVec3ToInt(ro.doors[i].voxelOwner.transform.position + (direction * v.voxelScale)))) {
                            ...
                            //we good!
                            Debug.Log("We don't have a collision with the door neighbours");
